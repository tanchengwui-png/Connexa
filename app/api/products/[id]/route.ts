...
export async function PATCH(request: NextRequest, context: RouteContext) {
  ...
  const product = await updateProduct(id, {
    ...
    financingTags: formData
      ...
      .map((value) => `${value}`.trim())
      .filter(Boolean),
    brochureName: removeBrochure ? null : uploadedBrochure?.name ?? toOptionalString(formData.get("existingBrochureName")),
    brochureUrl: removeBrochure ? null : uploadedBrochure?.publicPath ?? toOptionalString(formData.get("existingBrochureUrl")),
    ...
    imageUrls: [...existingImageUrls, ...uploadedGallery.map((item) => item.publicPath)]
  });
