...
export async function getContactsData(input?: {
  ...
  const total = await countVisibleContacts({ workspaceId, search, tags, ownerIds });
  ...
  const [contactsRows, agents, summary] = await Promise.all([
    findContactsWithOwner({ workspaceId, search, tags, ownerIds, limit: pageSize, offset }),
    findWorkspaceAgents(workspaceId),
    getContactDirectorySummary(workspaceId)
  ]);

  const contacts = await hydrateContacts(workspaceId, contactsRows);
...
export async function getAllVisibleContactsData(input?: {
  ...
  const [contactsRows, agents, summary] = await Promise.all([
    findAllVisibleContactsWithOwner({ workspaceId, search, tags, ownerIds }),
    findWorkspaceAgents(workspaceId),
    getContactDirectorySummary(workspaceId)
  ]);
...
async function hydrateContacts(
  workspaceId: string,
  contactsRows: Awaited<ReturnType<typeof findContactsWithOwner>>
) {
