...
powershell -NoProfile -Command ^
  ...
  "while ((Get-Date) -lt $deadline) { " ^
  "  try { " ^
  "    $response = Invoke-WebRequest -Uri 'http://localhost:3000/api/internal/outbound-message-jobs/heartbeat' -Method POST -Headers @{ 'x-worker-token' = '%OUTBOUND_WORKER_TOKEN%' } -ContentType 'application/json' -Body '{\"workerLabel\":\"launch-check\"}' -UseBasicParsing -TimeoutSec 5; " ^
...
start "Connexa Worker" powershell -NoExit -Command "$env:OUTBOUND_WORKER_TOKEN='%OUTBOUND_WORKER_TOKEN%'; $env:OUTBOUND_WORKER_URL='%OUTBOUND_WORKER_URL%'; Set-Location '%~dp0'; npm.cmd run worker:messages"
